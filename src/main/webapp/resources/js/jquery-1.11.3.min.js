...
(function($){
	$.fn.parallelRoll = function(options){
		...
		var w = $('.'+opts.boxName).find(opts.tagName).outerWidth(true); //计算元素的宽度  包括补白+边框
		...
		return this.each(function(){
			...
			$('.'+opts.prev).on('click',function(){
				var count = $('.'+opts.boxName).find(opts.tagName).length;
				if(count > opts.visual){
					flag ? left() : "";
				}
		...
		function right(){
			...
			for(var i = 0; i<opts.amount; i++){
				$(arr[i]).prependTo(_this).css({marginLeft : -w*(i+1)}).addClass("mleft");
			}	
...
//				flag = true;
//			});
			_this.find(".mleft").animate({marginLeft : 0},800,function(){
				_this.find(opts.tagName).removeAttr('style');
				_this.css({marginLeft:0});
				flag = true;
			}).removeClass("mleft");
